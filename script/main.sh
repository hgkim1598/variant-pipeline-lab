...
run_raw_qc() {
    ...
    local fastqc_zip_list="$STEP_WORK/fastqc_zips.txt"
    : > "$fastqc_zip_list"
    ...
    # ---- average Phred quality across all lanes -----------------------------
    # Reads the "Mean" column of FastQC's "Per base sequence quality" module
    # straight out of each fastqc_data.txt inside the zip (stdlib zipfile, no
    # new dependency) and averages it across bases, then across R1/R2 and all
    # lanes. This is an unweighted mean of per-base means, not a per-read
    # weighted mean — adequate for a one-line progress summary, not a
    # substitute for opening the FastQC report.
    if [[ -s "$fastqc_zip_list" ]]; then
        local avg_q
        avg_q=$("$(resolve_python)" - "$fastqc_zip_list" <<'PYAVGQ'
import sys, zipfile

zip_list_path = sys.argv[1]
means = []

with open(zip_list_path, encoding="utf-8") as fh:
    zip_paths = [line.strip() for line in fh if line.strip()]

for zp in zip_paths:
    try:
        with zipfile.ZipFile(zp) as zf:
            data_name = next((n for n in zf.namelist() if n.endswith("fastqc_data.txt")), None)
            if not data_name:
                continue
...
PYAVGQ
        ...
        if [[ -n "$avg_q" ]]; then
            step_metric avg_phred_quality "$avg_q" num
        else
            step_warning "FASTQC_QUALITY_UNAVAILABLE" \
                "Could not derive an average Phred quality from the FastQC output" \
                "Per-lane FastQC reports are unaffected and remain the source of truth" "true"
        fi
    fi
...
run_preprocessing() {
    ...
    local reads_before_total=0 reads_after_total=0
    ...
    if [[ "$TRIM_MODE" == "skip" ]]; then
        ...
        while IFS=$'\t' read -r sample lane rg lib plat pu fq1 fq2; do
            ...
            if [[ -s "$fastp_json" ]]; then
                ...
                [[ "$before" =~ ^[0-9]+$ ]] && reads_before_total=$(( reads_before_total + before ))
                [[ "$after"  =~ ^[0-9]+$ ]] && reads_after_total=$(( reads_after_total + after ))
            fi
        done < "$manifest"
        step_check_pass "trim_decision" "force — fastp applied to $lanes lane(s)"
        if (( reads_before_total > 0 )); then
            step_metric reads_before_total "$reads_before_total" num
            step_metric reads_after_total "$reads_after_total" num
        fi
    fi
...
run_intervar() {
    ...
    local intervar_report
    intervar_report=$(mktemp "${TMPDIR:-/tmp}/intervar-summary.XXXXXX") \
        || die "Could not create a temporary file for InterVar summary output"
    "$py" - "$result_file" "$summary_json" > "$intervar_report" <<'PYINTERVAR'
import json, os, sys
...
uncertain = sum(v for k, v in counts.items() if "Uncertain" in k)
benign = sum(v for k, v in counts.items() if "Pathogenic" not in k and "Uncertain" not in k and "Benign" in k)

doc = {
    "total_variants": total,
    "automated_evidence_counts": counts,
    "automated_pathogenic_or_likely_pathogenic": pathogenic,
    "automated_uncertain_significance": uncertain,
    "automated_benign_or_likely_benign": benign,
...
print(f"total={total} automated_pathogenic_or_lp={pathogenic} "
      f"uncertain={uncertain} benign={benign}")
print(f"METRIC intervar_total={total}")
print(f"METRIC intervar_pathogenic_lp={pathogenic}")
print(f"METRIC intervar_vus={uncertain}")
print(f"METRIC intervar_benign={benign}")
PYINTERVAR

    cat "$intervar_report" || true
    local key value
    while IFS='=' read -r key value; do
        [[ -n "$key" ]] && step_metric "$key" "$value" num
    done < <(grep '^METRIC ' "$intervar_report" | sed 's/^METRIC //')
    rm -f -- "$intervar_report"
